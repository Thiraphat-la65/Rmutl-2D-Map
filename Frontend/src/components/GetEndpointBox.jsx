...
const GetEndpointBox = ({ path, subtitle, fullPathParams, onCopySuccess }) => {
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      const copyData = {
        ...
        actionType: 'copy',
        actionDetails: `Copied GET link for ${subtitle || path}`,
        isSuccess: true,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error('Error fetching data:', err, { path: fullPath, param });
    }
  };

  const methodLabel = 'เรียกดูข้อมูล';
      ...
      <div className="flex justify-between items-center bg-orange-100 px-4 py-2 rounded-lg transform transition duration-300 hover:scale-101">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-3">
            <span className="bg-green-500 text-white px-3 py-1 rounded text-sm font-semibold">GET</span>
        ...
        <div className="p-4 bg-orange-50 border-t border-orange-200 duration-300 hover:scale-101">
          ...
          {isTryMode && (
            <div>
              <input
                type="text"
                placeholder="ใส่ค่าพารามิเตอร์ เช่น filter"
